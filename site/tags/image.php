...
kirbytext::$tags['image'] = [
  ...
  'html' => function ($tag) {
    ...
    $thumb = thumb($file, ['width' => 1000], false);
    ...
    if ($caption != '' || $file->sourcename()->isNotEmpty() || $file->sourceurl()->isNotEmpty()) {
        ...
        if ($file->sourcename()->isNotEmpty() || $file->sourceurl()->isNotEmpty()) {
            $html .= '[1000. Bild: ';
            if ($file->sourcename()->isNotEmpty()) {
                $html .= $file->sourcename();
            }
            if ($file->sourceurl()->isNotEmpty()) {
                $html .= ' (link: '.$file->sourceurl().' text: Zur Quelle popup: yes)';
            }
            $html .= ']';
        }
        $html .= '</figcaption>';
